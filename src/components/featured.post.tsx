...
import RecentlyCard from "./featured.post.card";
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-[56px] mt-[56px]">
            {data?.map((post: any, _idx: number) => {
              // Periksa apakah category adalah array, objek, atau string
              const categories = Array.isArray(post.fields.category)
                ? post.fields.category.map((cat: any) => cat.fields.category)
                : typeof post.fields.category === "string"
                ? [post.fields.category] // jika kategori berupa string, jadikan array
                : [post.fields.category?.fields?.category || ""];
